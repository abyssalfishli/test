...
var task17 *boomer.Task
var task18 *boomer.Task
...
func workerDo(request *http.Request) {
	...
	if err != nil {
		//dump.P(err)
		boomer.RecordFailure("http", "error1", 0.0, err.Error())
	} else {
		body, err := io.ReadAll(response.Body)
		if err != nil {
			dump.P(err)
			boomer.RecordFailure("http", "error2", 0.0, err.Error())
		} else {
			bodyStr := string(body)

			//解析json开始
			var responseJson = Response{}
			errUnmarshal := json.Unmarshal([]byte(bodyStr), &responseJson)
			if errUnmarshal != nil {
				dump.P(errUnmarshal.Error())
			}
			//解析json结束
			if responseJson.Message == "Hello Hyperf." {
				//dump.P(bodyStr)
				boomer.RecordSuccess("http", strconv.Itoa(response.StatusCode), elapsed.Nanoseconds()/int64(time.Millisecond), response.ContentLength)
			} else {
				//dump.P("Status Code:")
				//dump.P(response.StatusCode)
				//dump.P(bodyStr)
				boomer.RecordFailure("http", "error3", 0.0, bodyStr)
			}
		}
		...
		if closeError != nil {
			dump.P(closeError)
			boomer.RecordFailure("http", "error4", 0.0, closeError.Error())
		}
	}
	//=================================================自定义版结束=================================================
...
func mapi() {
	...
	method := "POST"
	request, err := http.NewRequest(method, address+url, nil)
	request.Host = "mapi.diasia.com"
	//req.header.add("user-agent", "myclient")
	request.Header.Add("AppKey", "MHvOuWEaOkLM9zdl")
	request.Header.Add("AppSecret", "3xvE8iGKXZ6SranminZF8UWCjFRC8l8T")
	if err != nil {
		//log.Fatalf("%v\n", err)
		dump.P(err)
	}
...
func main() {
	...
	flag.Parse()
	dump.P(taskType)
	//address = "http://e-nation.php:9501/v1/admin/upload/getCosDomain" 配套baidu方法
	address = "http://apiservice.golang:8081/api/test" //配套mapi方法-本地网络
	//address = "http://192.168.61.202/api/test" //配套mapi方法-公司网络
